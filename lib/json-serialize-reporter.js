...
function JsonSerializeReporter(runner, options) {
  ...
  if (options && options.reporterOptions) {
    var inOpts = options.reporterOptions;

    if (inOpts.stats !== undefined) {
      if (isString(inOpts.stats)) {
        switch (inOpts.stats.toLowerCase().trim()) {
          case 'false':
          case 'no':
          case 'off':
          case '0':
        ...
        opts.includeStats = Boolean(inOpts.stats);
      }
    }

    if (inOpts.replacer !== undefined) {
      if (isFunction(inOpts.replacer)) {
        opts.replacer = inOpts.replacer;
      }
    }

    if (inOpts.space !== undefined) {
      // eslint-disable-next-line no-restricted-globals
      if (!isNaN(Number(inOpts.space))) {
